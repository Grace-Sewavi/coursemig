...
const [isMenuOpen, setIsMenuOpen] = useState(false)
  ...
  return (
    <div className=' bg-div flex w-full static justify-between text-block py-6 px-4 md:px-8 bg-white drop-shadow-md'>
        <Link to ='/' className='md:w-40 h-10'>
        <img src= {logo} alt="" className='md:w-40'/>
        </Link>

    <div className='relative md:flex justify-between gap-1'>
      <i className='bx bx-search absolute pl-3 top-4 lg:block hidden  '></i>
      <input type="text" placeholder='Search here....' className=' text-sm md:text-md md:ml-0 ml-2 md:py-2 py-1.5 font-sans px-2 md:pl-10 rounded-xl border-2 bg-gray-200 focus:outline-none' />
      </div> 
      ...
      <ul className='hidden xl:flex items-center gap-6 font-sans text-base'>
        ...
        <i className='bx bx-menu xl:hidden block mt-1 ml-4 text-2xl cursor-pointer'
